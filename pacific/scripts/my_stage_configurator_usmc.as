...
// ------------------------------------------------------------------------------------------------
class MyStageConfiguratorUSMC : MyStageConfigurator {
	// ------------------------------------------------------------------------------------------------
	MyStageConfiguratorUSMC(GameModeInvasion@ metagame, MyMapRotator@ mapRotator) {
...
//		addStage(setupStageElk());
	...
	protected void setupMapView(MyStage@ stage) {
        ...
        }
		{
			...
			command.setStringAttribute("overlay_texture", "mapview_card_usmc.png");
			stage.m_extraCommands.insertLast(command);
		}
	}
	...
	protected Stage@ setupStage11() {
		...
		stage.m_maxSoldiers = 7 * 15;      // 105 units
		//stage.m_playerAiCompensation = 2;
		// stage.m_playerAiReduction = 3;   // not implemented!
		...
		stage.setPhaseController(PhaseControllerIsland1(m_metagame, enemySpawnCompensationFactor));
...
stage.addTracker(SpawnInBaseCallHandler(m_metagame, "vehicle_m4_e4.call", "vehicle_m4_e4_spawn.call", array<string> = {"Carrier"}, true, "vehicle"));
		...
		stage.m_soldierCapacityVariance = 0.7;      // was 0.6 in 1.65  (soldier cap raised by 2 at start)
		stage.m_maxSoldiers = 9 * 11;      // 99, was 108 units in 1.65
	//	stage.m_playerAiCompensation = 2;
    ...
    stage.addTracker(Spawner(m_metagame, 1, Vector3(736,15,346), 15, "regular"));        // prison hatch protector filler    
		...
		{				
			Faction f(getFactionConfigs()[0], createFellowCommanderAiCommand(0, 0.3, 0.2));    // was 0.25, 0.2 
      ...
      f.m_capacityOffset = 8;       // was 5
		...
		{
			Faction f(getFactionConfigs()[1], createCommanderAiCommand(1, 0.52, 0.35));
			f.m_overCapacity = 50;
...
stage.addTracker(SpawnInBaseCallHandler(m_metagame, "vehicle_m4_e4.call", "vehicle_m4_e4_spawn.call", array<string> = {"Carrier"}, true, "vehicle"));
		...
		stage.m_maxSoldiers = 9 * 9;      //  81 units
	//	stage.m_playerAiCompensation = 1;   // was 2 in 1.65
    // stage.m_playerAiReduction = 3.5;    // was 3 in 1.65
		stage.m_soldierCapacityVariance = 0.6;	 // was 0.75	

		stage.addTracker(Spawner(m_metagame, 1, Vector3(772,17,679), 15, "regular"));        // First base filler

		{				
			Faction f(getFactionConfigs()[0], createFellowCommanderAiCommand(0, 0.3, 0.2));      
			f.m_capacityOffset = 20;        // was 12
			f.m_capacityMultiplier = 0.85;
			f.m_bases = 1;
			f.m_overCapacity = 10;			 
			stage.m_factions.insertLast(f);
		...
		{
			Faction f(getFactionConfigs()[1], createCommanderAiCommand(1, 0.6 , 0.25));     // was 0.5, 0.35
			f.m_overCapacity = 80;       // was 70
			f.m_capacityMultiplier = 1.0;
			stage.m_factions.insertLast(f);
		}
		...
		
		{
			XmlElement command("command");
			command.setStringAttribute("class", "update_map_view");
			command.setStringAttribute("overlay_texture", "map_usmc.png");
			stage.m_extraCommands.insertLast(command);
		}
		{
            XmlElement command("command");
    ...
    // stage.m_playerAiReduction = 2;
		stage.m_soldierCapacityVariance = 0.65;
		...
		{
			Faction f(getFactionConfigs()[1], createCommanderAiCommand(1, 0.43, 0.23));
			f.m_overCapacity = 50;
			f.m_capacityMultiplier = 1.0;
			stage.m_factions.insertLast(f);
		}
...
stage.addTracker(SpawnInBaseCallHandler(m_metagame, "usf_vehicle_m3_halftrack.call", "usf_vehicle_m3_halftrack_spawn.call", array<string> = {"Attack Ship"}, true, "vehicle"));
		...
		stage.addStartComment(Comment("map start with 1 base, part 2", 5.0));
		stage.addStartComment(Comment("map start with 1 base, part 3", 5.0));
		stage.m_maxSoldiers = 9 * 14;      // 126 units
	//	stage.m_playerAiCompensation = 2;
		// stage.m_playerAiReduction = 2;
		stage.m_soldierCapacityVariance = 0.5;
        
		stage.addTracker(Spawner(m_metagame, 1, Vector3(256,10,590), 15, "regular"));        // First base filler  
		stage.addTracker(Spawner(m_metagame, 1, Vector3(360,10,563), 15, "regular"));        // radio jammer filler      		
		...
		{
			Faction f(getFactionConfigs()[1], createCommanderAiCommand(1, 0.5, 0.3));
			f.m_overCapacity = 30;
			f.m_capacityOffset = 10;
			f.m_capacityMultiplier = 1.0;
			stage.m_factions.insertLast(f);
...
stage.addTracker(SpawnInBaseCallHandler(m_metagame, "usf_vehicle_m3_halftrack.call", "usf_vehicle_m3_halftrack_spawn.call", array<string> = {"Attack Ship"}, true, "vehicle"));
		...
		stage.m_maxSoldiers = 10 * 16;       // 160 units
	//	stage.m_playerAiCompensation = 3;    // was 2 in 1.60
		// stage.m_playerAiReduction = 2.4;		   
		
		stage.m_soldierCapacityVariance = 0.8;		 
		...
		{
			Faction f(getFactionConfigs()[1], createCommanderAiCommand(1, 0.45, 0.32));
			f.m_overCapacity = 70;
			f.m_capacityOffset = 10;      
			f.m_capacityMultiplier = 1.0;
			stage.m_factions.insertLast(f);
    ...
    stage.addTracker(Spawner(m_metagame, 1, Vector3(749,15,185), 15, "regular"));        // Headquarter base filler       
		
		{				
			Faction f(getFactionConfigs()[0], createFellowCommanderAiCommand(0, 0.35, 0.2));
			f.m_capacityOffset = 5;             // 5
			f.m_capacityMultiplier = 0.85;       // was 0.8
			f.m_bases = 1;
			f.m_overCapacity = 15;				
			stage.m_factions.insertLast(f);
		...
		{
			Faction f(getFactionConfigs()[1], createCommanderAiCommand(1, 0.55, 0.35));                //  0.45, 0.32
			f.m_overCapacity = 100;            // 70
			f.m_capacityOffset = 0;           // 10
			f.m_capacityMultiplier = 1.0;
			stage.m_factions.insertLast(f);
		}
	...
	protected Stage@ setupStageFinal() {
		...
		stage.addTracker(SpawnInBaseCallHandler(m_metagame, "usmc_vehicle.call", "usmc_vehicle_spawn.call", array<string> = {"the_armory","submarine_pens","warehouses","the_heights"}, true, "vehicle"));
		stage.addTracker(SpawnInBaseCallHandler(m_metagame, "usmc_vehicle1.call", "usmc_vehicle1_spawn.call", array<string> = {"the_armory","submarine_pens","warehouses","the_heights"}, true, "vehicle"));
		stage.addTracker(SpawnInBaseCallHandler(m_metagame, "usf_vehicle_m4_sherman.call", "usf_vehicle_m4_sherman_spawn.call", array<string> = {"the_armory","submarine_pens","warehouses","the_heights"}, true, "vehicle"));
		stage.addTracker(SpawnInBaseCallHandler(m_metagame, "usf_vehicle.call", "usf_vehicle_spawn.call", array<string> = {"the_armory","submarine_pens","warehouses","the_heights"}, true, "vehicle"));
		stage.addTracker(SpawnInBaseCallHandler(m_metagame, "usf_vehicle_m10.call", "usf_vehicle_m10_spawn.call", array<string> = {"the_armory","submarine_pens","warehouses","the_heights"}, true, "vehicle"));
		...
		stage.addTracker(SpawnInBaseCallHandler(m_metagame, "usf_vehicle_m4a3e8.call", "usf_vehicle_m4a3e8_spawn.call", array<string> = {"the_armory","submarine_pens","warehouses","the_heights"}, true, "vehicle"));
		
		stage.addTracker(SpawnInBaseCallHandler(m_metagame, "usf_vehicle_armoury_marine.call", "usf_vehicle_armoury_marine_spawn.call", array<string> = {"the_armory","submarine_pens","warehouses","the_heights"}, true, "vehicle"));
		stage.addTracker(SpawnInBaseCallHandler(m_metagame, "ija_vehicle_armoury_marine.call", "ija_vehicle_armoury_marine_spawn.call", array<string> = {}, true, "vehicle"));
	...
	protected void setupTransports() {
		...
		addTransport("island2", "hitbox_extraction1_usmc", "island1");
		...
		addTransport("island3", "hitbox_extraction2_usmc", "island2");
		addTransport("island3", "hitbox_extraction4_usmc", "island4");
		...
		addTransport("island3", "hitbox_extraction9_usmc", "island9");
		
		addTransport("island4", "hitbox_extraction2_usmc", "island2");
		addTransport("island4", "hitbox_extraction3_usmc", "island3");
		...
		addTransport("island9", "hitbox_extraction2_usmc", "island2");
		addTransport("island9", "hitbox_extraction3_usmc", "island3");
		addTransport("island9", "hitbox_extraction4_usmc", "island4");
		...
		addTransport("island5", "hitbox_extraction3_usmc", "island3");
		addTransport("island5", "hitbox_extraction4_usmc", "island4");
	    addTransport("island5", "hitbox_extraction8_usmc", "island8");
		...
		addTransport("island6", "hitbox_extraction3_usmc", "island3");
		addTransport("island6", "hitbox_extraction4_usmc", "island4");
		addTransport("island6", "hitbox_extraction5_usmc", "island5");
	    addTransport("island6", "hitbox_extraction8_usmc", "island8");
		addTransport("island6", "hitbox_extraction7", "island7");
		addTransport("island6", "hitbox_extraction9", "island9");
		
		addTransport("island8", "hitbox_extraction3_usmc", "island3");
		addTransport("island8", "hitbox_extraction4_usmc", "island4");
		addTransport("island8", "hitbox_extraction5_usmc", "island5");
	    addTransport("island8", "hitbox_extraction6_usmc", "island6");
		addTransport("island8", "hitbox_extraction7", "island7");
		addTransport("island8", "hitbox_extraction9_usmc", "island9");
