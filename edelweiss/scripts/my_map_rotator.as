...
class MyMapRotator : MapRotatorCampaign {
	...
	// --------------------------------------------
    void restartMap() {
		// using a custom map restart
		// - going via loading screen to fully reload initial state from objects.svg etc
		// - in Edelweiss scripts are doing more dynamic stuff than in vanilla and "shallow" map restart
		//   can leave some modified values in the system that do not get reset to initial state
		// - another option would be to manually reset the relevant values at start of map / first phase
		//   but this is safer and doesn't need care, although slower to process
