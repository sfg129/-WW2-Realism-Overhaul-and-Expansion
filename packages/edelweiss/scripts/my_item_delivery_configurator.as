...
#include "my_resource_unlocker.as"
#include "unlock_customizations.as"
#include "unlock_firemodes.as"

// ------------------------------------------------------------------------------------------------
class MyItemDeliveryConfigurator : ItemDeliveryConfigurator {
	...
	void refresh() {
		...
		refreshEnemyItemDeliveryObjectives();
	}
	...
	protected void setupLaptopUnlocks() {
		...
		{
			 deliveryList.insertLast(Resource("beret.carry_item", "carry_item"));
		}

		dictionary unlockList;
		{
			string target = "beret.carry_item";
			array<Resource@>@ list = getUnlockItemList2();
	...
	protected void setupBriefcaseUnlocks() {
		...
		string thanks = "item objective thanks";
		// customized resource unlocker that allows overriding items in the unlock list based on map
		MyResourceUnlocker@ unlocker = MyResourceUnlocker(m_metagame, 0, unlockList, getUnlockCustomizations(), getUnlockFiremodes(), m_metagame, "", thanks);
	...
	// ----------------------------------------------------  
	protected void setupEnemyItemUnlocks() {
		array<ItemDeliveryObjective@> objectives = createEnemyItemDeliveryObjectives();
	...
	// ----------------------------------------------------
	protected array<Resource@>@ getEnemyItemDeliverables() const {
		array<Resource@> results;

		// get the stuff we have in stock
		array<const XmlElement@> own;
		{
			merge(own, getFactionDeliverables(m_metagame, 0, "weapon", "weapons"));
			merge(own, getFactionDeliverables(m_metagame, 0, "grenade", "grenades"));
			merge(own, getFactionDeliverables(m_metagame, 0, "carry_item", "carry_items"));
		}
	...
	// ----------------------------------------------------
	protected array<ItemDeliveryObjective@>@ createEnemyItemDeliveryObjectives() const {
		_log("createEnemyItemDeliveryObjectives", 1);
		...
		array<Resource@>@ enemyItemResources = getEnemyItemDeliverables();
		for (uint i = 0; i < enemyItemResources.size(); ++i) {
			Resource@ resource = enemyItemResources[i];
			_log("enemy unlock target " + resource.m_key, 1);
			array<Resource@> deliveryList = {resource};
			// delivering certain amount of specific item unlocks that particular item
			dictionary unlockList = {
			...
			MyResourceUnlocker@ unlocker = MyResourceUnlocker(m_metagame, 0, unlockList, getUnlockCustomizations(), getUnlockFiremodes(), m_metagame, /* custom stat tracker tag */ "enemy_weapon_delivered");

			int amount = 5;

			ItemDeliveryObjective objective(m_metagame, 0, deliveryList, m_itemDeliveryOrganizer, unlocker, instructions, "", thanks, thanksIncomplete, amount, 0, 50, getUnlockFiremodes());
	...
	// ----------------------------------------------------
	protected void refreshEnemyItemDeliveryObjectives() {
		// only creates ones that don't already exist
		array<ItemDeliveryObjective@>@ newObjectives = createEnemyItemDeliveryObjectives();
	...
	// These are implemented in WH/USF/UKF specific classes
	// --------------------------------------------
