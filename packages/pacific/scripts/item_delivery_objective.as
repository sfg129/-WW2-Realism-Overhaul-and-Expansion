...
class ItemDeliveryObjective : Objective {
	...
	ItemDeliveryObjective(Metagame@ metagame, int factionId, const array<Resource@>@ itemList, ItemDeliveryOrganizer@ organizer, ResourceUnlocker@ unlocker, string instructions, 
		string thanks, string thanksIncomplete, int deliveryAmount, ItemDeliveryRewarder@ rewarder, const dictionary@ firemodes = dictionary = {}) {
		//I just stick the firemodes stuff at the end
		//and yeah, I'm not making a separate ItemDeliveryObjective, too many instances to replace, means too many options for errors -G
	...
	ItemDeliveryObjective(Metagame@ metagame, int factionId, const array<Resource@>@ itemList, ItemDeliveryOrganizer@ organizer, ResourceUnlocker@ unlocker, string instructions, 
		...
		float rewardForCompletion = 100.0, const dictionary@ firemodes = dictionary = {}) {
		//same, lazy to check which one is relevant for the firemodes stuff -G
	...
	// ----------------------------------------------------G
	protected string findFiremode(string source) {
		string itemKey = source;
		//first we check if the specified weapon has an alt firemode (bayonet)		
		//if not then it is still possible that we delivered the bayonet variant, so we have to reverse search for the basic version
		if (!m_firemodes.exists(itemKey)){
			string target;
			array<string>@ firemodeKeys = m_firemodes.getKeys();
			for (int i = 0; i < firemodeKeys.length(); ++i) {			
				m_firemodes.get(firemodeKeys[i], target);			
				if (target == itemKey){
					//we found it, we handle it as if the basic variant got delivered instead of the bayonet version
					itemKey = firemodeKeys[i];
					return itemKey;
				}
			}
		}
	...
	protected void handleItemDropEvent(const XmlElement@ event) {
		...
		// ensure it's the wanted item, checking for bayonet variant too -G
		string itemKey = event.getStringAttribute("item_key");
		itemKey = findFiremode(itemKey);		
		const Resource@ targetItem = getItemResource(itemKey);
		...
		if (m_deliveryAmount > 0) {
			...
			if (m_deliveryAmount <= 0) {
				...
				if (m_unlocker !is null) {
					...
					//my_resource_unlocker will handle unlocking the bayonet variant too -G
				}
