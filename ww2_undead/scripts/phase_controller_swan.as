...
class SwanPhase0 : DefPhaseBase {
	...
	// --------------------------------------------
	SwanPhase0(GameModeInvasion@ metagame, PhaseController@ controller, float enemySpawnCompensationFactor) {
	...
	void start() {
		DefPhaseBase::start();
		_log("SwanPhase0 starting");

		if (_logger.m_logLevel >= 1) {
			announce("TEST: SwanPhase0 starting");
		}
		
		array<const XmlElement@> baseList = getBases(m_metagame);
		for (uint i = 0; i < baseList.size(); ++i) {
			if (baseList[i].getStringAttribute("key") == "Research_Center") {
				continue;
			}
	...
	void update(float time) {
		...
		m_timer2 -= time;
		if (m_timer2 <= 0.0f) {
			m_metagame.getTaskSequencer().add(AnnounceTask(m_metagame, 5.0, 0, "undead swan reminder"));
...
// --------------------------------------------
class SwanPhaseBoss : DefPhaseBase {
	...
	// --------------------------------------------
	SwanPhaseBoss(GameModeInvasion@ metagame, PhaseController@ controller, float enemySpawnCompensationFactor) {
		super(metagame, controller, enemySpawnCompensationFactor);
	}
	...
	void start() {
		...
		m_metagame.getTaskSequencer().add(AnnounceTask(m_metagame, 5.0, 0, "undead swan gate open pt 1"));
		m_metagame.getTaskSequencer().add(AnnounceTask(m_metagame, 5.0, 0, "undead swan gate open pt 2"));
		...
		m_position = Vector3(870.0, 6.0, 82.0);			
		...
		if (_logger.m_logLevel >= 1) {
			announce("TEST: SwanPhaseBoss starting");
		}
	...
	void update(float time) {
		...
		m_timer2 -= time;
		if (m_timer2 <= 0.0f) {
			m_metagame.getTaskSequencer().add(AnnounceTask(m_metagame, 5.0, 0, "undead swan gate open pt 3 reminder"));
			m_timer2 = m_reminderInterval;
		}
	}
...
// --------------------------------------------
class PhaseControllerSwan : PhaseControllerBase {

	// --------------------------------------------
	PhaseControllerSwan(GameModeInvasion@ metagame, float enemySpawnCompensationFactor) {
	...
	void reset() {
		...
		m_phases.insertLast(SwanPhase0(m_metagame, this, m_enemySpawnCompensationFactor));	
		m_phases.insertLast(SwanPhaseBoss(m_metagame, this, m_enemySpawnCompensationFactor));
	}
    ...
    protected void handleChatEvent(const XmlElement@ event) {
		...
		if (checkCommand(message, "end_phase")) {
			...
			m_metagame.getComms().send("<command class='update_base' owner_id='1' base_key='Research_Center' />");
		} else if (checkCommand(message, "boss_2")) {
			m_metagame.getComms().send("<command class='update_base' owner_id='2' base_key='Research_Center' />");
		}
	}
}