...
class SealionPhase0 : DefPhaseBase {
	...
	void start() {
		...
		for (uint i = 0; i < baseList.size(); ++i) {
			if (baseList[i].getStringAttribute("key") == "citadel") {
				continue;
			}
...
class SealionPhaseBoss : DefPhaseBase {
	...
	void start() {
		...
		m_position = Vector3(124.0, 6.0, 873.0);
...

		{
			...
			// populating the barracks in front of the Citadel entrance
			if (m_bossFaction == 1) {
				//m_metagame.addTracker(Spawner(m_metagame, 1, m_position, 5, "nachtjaeger_fsj"));
				command.setStringAttribute("instance_class", "call");
				command.setStringAttribute("instance_key", "ww2_undead_axr_para_large.call");
			} else {
				...
				command.setStringAttribute("instance_key", "ww2_undead_zombie_overlord_spawner.vehicle");
			}
			command.setStringAttribute("position", "248 6 844");
			m_metagame.getComms().send(command);
			command.setStringAttribute("position", "267 6 889");
			m_metagame.getComms().send(command);
			command.setStringAttribute("position", "229 6 792");
			m_metagame.getComms().send(command);
		}
	}
...
class PhaseControllerSealion : PhaseControllerBase {
    ...
    protected void handleChatEvent(const XmlElement@ event) {
		...
		if (checkCommand(message, "end_phase")) {
			...
			m_metagame.getComms().send("<command class='update_base' owner_id='1' base_key='citadel' />");
		} else if (checkCommand(message, "boss_2")) {
			m_metagame.getComms().send("<command class='update_base' owner_id='2' base_key='citadel' />");
		}
	}
}